...
void TfePcapFreeLibrary(void)
{
    if (pcap_library) {
        if (!FreeLibrary(pcap_library)) {
            printf("FreeLibrary WPCAP.DLL failed!");
        }
        pcap_library = NULL;
...
#define GET_PROC_ADDRESS_AND_TEST( _name_ ) \
    ...
    if (!p_##_name_ ) { \
        printf("GetProcAddress " #_name_ " failed!"); \
        TfePcapFreeLibrary(); \
        return FALSE; \
    } 
...
BOOL TfePcapLoadLibrary(void)
{
    if (!pcap_library) {
        ...
        if (!pcap_library) {
            printf("LoadLibrary WPCAP.DLL failed!" );
            return FALSE;
        }
...
int tfe_arch_enumadapter_open(void)
{
    ...
    {
        printf("ERROR in TfeEnumAdapterOpen: pcap_findalldevs: '%s'", TfePcapErrbuf);
        return 0;
    }

	if (!TfePcapAlldevs) {
        printf("ERROR in TfeEnumAdapterOpen, finding all pcap devices - "
...
BOOL TfePcapOpenAdapter(const char *interface_name) 
{
    ...
    if ( TfePcapFP == NULL)
    {
        printf("ERROR opening adapter: '%s'", TfePcapErrbuf);
    ...
    {
        printf("WARNING: Setting PCAP to non-blocking failed: '%s'", TfePcapErrbuf);
    }

	/* Check the link layer. We support only Ethernet for simplicity. */
	if((*p_pcap_datalink)(TfePcapFP) != DLT_EN10MB)
	{
		printf("ERROR: TFE works only on Ethernet networks.");
		tfe_enumadapter_close();
        return FALSE;
	}
...
void tfe_arch_pre_reset( void )
{
    ...
    fprintf( g_fh, "tfe_arch_pre_reset()." );
#endif
}
...
{
#ifdef TFE_DEBUG_ARCH
    fprintf( g_fh, "tfe_arch_post_reset()." );
#endif
}
...
{
#ifdef TFE_DEBUG_ARCH
    fprintf( g_fh, "tfe_arch_activate()." );
#endif
...
{
#ifdef TFE_DEBUG_ARCH
    fprintf( g_fh, "tfe_arch_deactivate()." );
#endif
}
...
void tfe_arch_set_mac( const BYTE mac[6] )
{
    ...
    fprintf( g_fh, "New MAC address set: %02X:%02X:%02X:%02X:%02X:%02X.",
...
{
#if defined(TFE_DEBUG_ARCH) || defined(TFE_DEBUG_FRAMES)
    fprintf( g_fh, "New hash filter set: %08X:%08X.",
...
{
#ifdef TFE_DEBUG_ARCH
    fprintf( g_fh, "tfe_arch_receive_remove_committed_frame()." );
#endif
}
*/
...
#ifdef TFE_DEBUG_ARCH
    fprintf( g_fh, "tfe_arch_receive_frame() called, returns %d.", ret );
#endif
...
{
#ifdef TFE_DEBUG_ARCH
    fprintf( g_fh, "tfe_arch_transmit() called, with: "
    ...
    if ((*p_pcap_sendpacket)(TfePcapFP, txframe, txlength) == -1) {
        printf("WARNING! Could not send packet!");
    }
}
...
#ifdef TFE_DEBUG_ARCH
    fprintf( g_fh, "tfe_arch_receive() called, with *plen=%u.", *plen );
#endif
...
char * tfe_getadapter(struct in_addr addr)
{
  ...
  if((*p_pcap_findalldevs)(&interfaces, error) == -1) {
    return retval;
  }
  ...
  while(interfaces != NULL) 
  {
    ...
    if(interfaces->addresses != NULL) {
			for(paddr = interfaces->addresses; paddr != NULL; paddr = paddr->next) {
  ...
  if(!iffound) {
    printf("no interface found with ip addr specified on cmdline\n");
    return retval;
  }
