...
const char *g_gsport_default_paths[] = { // probably overkill on the paths
  "",
  "./",
  "${HOME}/",
  "${PWD}/",
...
const char g_gsport_version_str[] = "0.4";
...
void setup_gsport_file(char *outname, int maxlen, int ok_if_missing, int can_create_file, const char **name_ptr) {
  ...
  if(can_create_file) {
    ...
    // This is often wrong in OS X. /Applications/GSport.app is
    ...
    // However, GSport is promoting a notion of Config-as-a-VM
    ...
    clear_fatal_logs();
    setup_gsport_file(outname, maxlen, ok_if_missing,       can_create_file, name_ptr);
